...
# Helper function for backing up to Supabase
def backup_to_supabase(file_content, filename, user_id, report_type):
    """
    ...
    Args:
        ...
        report_type: Type of report (e.g., 'courses', 'students', 'reviews')
    ...
    try:
        ...
        # Create a structured path in the bucket
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        storage_path = f"backups/{report_type}/{user_id}/{timestamp}_{filename}"
        ...
        print(f"✅ Backup uploaded successfully: {storage_path}")
        return {
            'success': True,
            'path': storage_path,
            'response': response
...
def course_list(request, student_id):
    ...
    if request.method == 'POST':
        ...
        if student in course.students.all():
            ...
            course.students.add(student)
        return redirect('course_list', student_id=student.id)
...
def admin_create_course(request):
    if request.method == 'POST':
        ...
        code = request.POST.get('code')
        credit_units = request.POST.get('credit_units')
        lecturer_name = request.POST.get('lecturer')
...
# CSV DOWNLOAD VIEWS WITH SUPABASE BACKUP
...
def download_courses_csv(request):
    ...
    writer = csv.writer(response)
    csv_content = [['Course Name', 'Code', 'Credit Units', 'Lecturer']]

    for course in Course.objects.all():
        row = [course.name, course.code, course.credit_units, course.lecturer]
        writer.writerow(row)
    ...
    try:
        ...
        backup_writer = csv.writer(csv_buffer)
        backup_writer.writerows(csv_content)
        csv_bytes = csv_buffer.getvalue().encode('utf-8')
        
        backup_to_supabase(csv_bytes, filename, request.user.id, 'courses')
    except Exception as e:
        print(f"Backup failed but download continues: {str(e)}")
...
def download_students_per_course_csv(request):
    response = HttpResponse(content_type='text/csv')
    filename = 'students_per_course.csv'
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    
    writer = csv.writer(response)
    csv_content = [['Course', 'Student Name', 'Email']]

    for course in Course.objects.all():
        for student in course.students.all():
            row = [course.name, student.name, student.email]
            writer.writerow(row)
            csv_content.append(row)
    ...
    try:
        ...
        backup_to_supabase(csv_bytes, filename, request.user.id, 'students_per_course')
    except Exception as e:
        print(f"Backup failed but download continues: {str(e)}")
...
def download_reviews_csv(request):
    response = HttpResponse(content_type='text/csv')
    filename = 'course_reviews.csv'
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    
    writer = csv.writer(response)
    csv_content = [['Course', 'Student', 'Rating', 'Comment']]

    for review in CourseReview.objects.all():
        row = [review.course.name, review.student.name, review.rating, review.comment]
        writer.writerow(row)
        csv_content.append(row)
    ...
    try:
        ...
        backup_to_supabase(csv_bytes, filename, request.user.id, 'reviews')
    except Exception as e:
        print(f"Backup failed but download continues: {str(e)}")
...
def download_summary_csv(request):
    ...
    writer = csv.writer(response)
    csv_content = [['Summary Type', 'Count']]

    rows = [
        ['Total Courses', Course.objects.count()],
        ['Total Students', sum(c.students.count() for c in Course.objects.all())],
        ['Total Lecturers', len(set(c.lecturer for c in Course.objects.all() if c.lecturer))],
        ['Total Reviews', CourseReview.objects.count()]
    ]
    ...
    try:
        ...
        backup_to_supabase(csv_bytes, filename, request.user.id, 'summary')
    except Exception as e:
        print(f"Backup failed but download continues: {str(e)}")
